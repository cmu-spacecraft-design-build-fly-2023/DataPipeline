...
def rotate(raster, bboxes):
    ...
    for idx, box in bboxes.items():
        ...
        box_points.append(list([int(cx), int(cy)]))
        box_points.append(list([int(xmin), int(ymin)]))
        box_points.append(list([int(xmax), int(ymax)]))
        ...
        rot_boxes.update({idx:[ul_corner, br_corner, rot_box[0]]})
...
def process_single_image(file, data_path, all_landmarks, region, val_set, test_set):
    ...
    # IF TEST DATA (rotate image and boxes for landsat)
    if test_set is True:
...
def process_data_with_pool(args):
    ...
    region = args.region
    anno_file = os.path.join(landmark_path, f"{region}_top_salient.csv")
    print("ANNO FILE: ", anno_file)
    
    # Make sure the images output directory exists
    if val_set is True:
        save_path = os.path.join(output_path, "val/images")
    elif test_set is True:
        save_path = os.path.join(output_path, "test/images")
    else:
    ...
    if val_set is True:
        ...
        label_path = os.path.join(output_path, "test/labels")
    else:
    ...
    with open(anno_file, 'r') as f:
        ...
        for i, row in enumerate(csvReader):
            ...
            all_landmarks[i] = [[min_long, max_lat], [max_long, min_lat], [centroid_lon, centroid_lat]]

    files = glob.glob(os.path.join(data_path, '*.tif'))

    # Use process_map to process images with progress tracking
    process_args = [(file, save_path, all_landmarks, region, val_set, test_set) for file in files]
    ...
    if val_set is not True and test_set is not True:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--test", type=bool, help="Flag for creating rotated Landsat validation dataset")
